/* ═══════════════════════════════════════════════════
   ...
   Fix #1: Only persist session (login) and anon_users.
   All other data fetched from Google Sheet.
   ═══════════════════════════════════════════════════ */
...
const DB = {
  ...
  // Fix #1: Only these persist locally for login memory
  ...
  // Reviews/ranking: local write-through cache (populated by sheet fetch)
  ...
  // Poll votes: local dedup only (voterKey::type::pollId -> optIdx)
  getPollVotes()  { return DB.get('poll_votes')     || {}; },
  setPollVotes(v) { DB.set('poll_votes', v); },
  // Complaints/updates: fetched from sheet, local write-through
...
function doLogout() { DB.set('session', null); currentUser = null; window.location.href = 'login.html'; }

function getGuestId() {
  let gid = localStorage.getItem('niser_guest_id');
  if (!gid) { gid = 'guest_' + Date.now() + '_' + Math.random().toString(36).slice(2,9); localStorage.setItem('niser_guest_id', gid); }
  return gid;
}
function getVoterKey() { return currentUser ? currentUser.email : getGuestId(); }
...
function toggleHamburger(navId) { const el = document.getElementById(navId); if (el) el.classList.toggle('open'); }
...
function getActiveEmailSystem() {
  ...
  const today = new Date(), lastReset = stats.lastReset ? new Date(stats.lastReset) : null;
  if (!lastReset || (today.getDate() >= 14 && (today.getMonth() !== lastReset.getMonth() || today.getFullYear() !== lastReset.getFullYear()))) {
    if (!lastReset || today.getDate() === 14) { stats.counts = [0,0,0,0]; stats.lastReset = today.toISOString(); DB.setEmailStats(stats); }
  }
  return EMAILJS_SYSTEMS[stats.currentSystem % EMAILJS_SYSTEMS.length];
}
function recordEmailSent() {
  const stats = DB.getEmailStats(); const sys = stats.currentSystem % EMAILJS_SYSTEMS.length;
...
function getEmailSystemStatus() {
  const stats = DB.getEmailStats();
  return EMAILJS_SYSTEMS.map((s, i) => ({ num: i+1, serviceId: s.serviceId, limit: s.limit, used: stats.counts[i]||0, remaining: Math.max(0, s.limit-(stats.counts[i]||0)), active: (stats.currentSystem % EMAILJS_SYSTEMS.length) === i }));
}
...
function renderHeader(activePage) {
  ...
  } else { userHTML = `<a class="nav-btn primary" href="login.html">Log In</a>`; }
  const navLinks = [
  ...
  el.innerHTML = `<a class="header-brand" href="index.html"><div class="header-logo">N</div><div><div class="header-title">NISER Mess</div><div class="header-sub">NISER · Bhubaneswar</div></div></a>
    <nav class="header-nav" id="main-nav">${navHTML}<div id="user-area">${userHTML}</div></nav>
...
function renderFooter(extra) {
  ...
  el.innerHTML = `<strong>NISER Mess Portal</strong> · National Institute of Science Education and Research, Bhubaneswar
